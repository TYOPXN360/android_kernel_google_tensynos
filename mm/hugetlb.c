...
static int __huge_pmd_unshare(struct mm_struct *mm, struct vm_area_struct *vma,
		unsigned long addr, pte_t *ptep, bool check_locks);
...
{
	return __huge_pmd_unshare(mm, vma, addr, ptep, /*check_locks=*/true);
}
				   ...
				   bool take_locks)
{
	...
	for (address = start; address < end; address += PUD_SIZE) {
		...
		if (!ptep)
			continue;
		ptl = huge_pte_lock(h, mm, ptep);
		__huge_pmd_unshare(mm, vma, address, ptep, take_locks);
		spin_unlock(ptl);
	}
	flush_hugetlb_tlb_range(vma, start, end);
	if (take_locks) {
