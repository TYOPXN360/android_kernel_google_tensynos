...
static int i2c_register_adapter(struct i2c_adapter *adap)
{
	...
	/*
	 * ANDROID: per-adapter debugfs (adap->debugfs / i2c_debugfs_root) is
	 * reverted for KMI stability, so the upstream debugfs_create_dir() is
	 * intentionally omitted here. The device_add()/idr_replace() split from
	 * the upstream "fix adapter registration race" series must stay: without
	 * device_add() the adapter has no sysfs node and every OF-declared child
	 * client fails kobject_add with -ENOENT.
	 */
